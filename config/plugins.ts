export default ({ env }) => {
  ...
  const endpoint = env('STRAPI_PLUGIN_UPLOAD_PROVIDER_OPTIONS_ENDPOINT');
  const region = env('STRAPI_PLUGIN_UPLOAD_PROVIDER_OPTIONS_REGION');
  const bucket = env('STRAPI_PLUGIN_UPLOAD_PROVIDER_OPTIONS_BUCKET');
      ...
      config: {
        ...
        providerOptions: {
          ...
          s3Options: {
            credentials: {
              accessKeyId: env(
                'STRAPI_PLUGIN_UPLOAD_PROVIDER_OPTIONS_ACCESS_KEY_ID',
              ),
              secretAccessKey: env(
